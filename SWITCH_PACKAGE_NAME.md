...
这个代码库是 **Excalidraw 的源码**，使用 `@excalidraw` 作为包名。

当你需要发布**二次开发版本**时，需要将包名切换为 `@excalidraw`。

## 使用场景

### 日常开发
- **保持使用 `@excalidraw`**：源码开发时使用原始包名
- 这是默认状态，与上游仓库保持一致

### 发布二开版本
- **切换到 `@excalidraw`**：发布前切换包名
- 发布完成后可以切换回 `@excalidraw` 继续开发

## 快速使用

### 切换到 @excalidraw（日常开发，默认状态）
```bash
...
### 日常开发流程（使用 @excalidraw）

1. **保持源码状态**：代码库默认使用 `@excalidraw`，与上游仓库保持一致
...
3. **发布后切换回来**：发布完成后记得切换回 `@excalidraw`，保持与上游一致
4. **排除目录**：脚本会自动排除 `node_modules`、`dist`、`types`、`.git` 等目录

### 发布脚本说明

现有的 `yarn release` 脚本已经配置为发布 `@excalidraw` 包（见 `scripts/release.js` 中的 `PACKAGE_SCOPE`）。

**重要**：使用 `yarn release` 前，请确保已经切换到 `@excalidraw`：
```bash
yarn switch:modify
yarn release --tag=latest --version=0.19.0
```
